...
APP_VERSION='3.11.3'
MARKER='CSM_RELEASE_IDENTITY_3_11_3'
DEV_VERSION='1.3.0'
TAX_VERSION='1.0.2'
DIFAL_VERSION='1.1.1'
BUILD_NAME='Consulta NF-e 3.11.3 Botao Visivel + XML Seguro + Abertura Automatica + PostInstall Fresh + DIFAL ICMS ST 1.1.1 + Entender a Tributacao 1.0.2 + Motor Fiscal de Devolucao 1.3'
OLD=r'(?:3\.7\.8|3\.8\.0|3\.8\.1|3\.9\.0|3\.9\.1|3\.9\.2|3\.9\.3|3\.9\.4|3\.9\.5|3\.9\.6|3\.10\.0|3\.10\.1|3\.10\.2|3\.11\.0|3\.11\.1|3\.11\.2)(?:\.\d+)?'
...
def main():
    ...
    if t.count(expected)<2:raise SystemExit('Modal Sobre nao esta fixado em 3.11.3')
    if MARKER not in t:app.write_text(t.rstrip()+f'\n// {MARKER} — {BUILD_NAME}\n',encoding='utf-8',newline='\n')
    csm.mkdir(parents=True,exist_ok=True);root=web.parent.parent
    (root/'VERSION.txt').write_text(f'CSM Visualizador XML {APP_VERSION}\nConsulta NF-e por chave com botão visível e persistente\nXML seguro, Excel/CSV bloqueados e abertura automática\nPrimeira abertura pós-instalação em modo fresh\nDIFAL / ICMS ST {DIFAL_VERSION}\nMotor Fiscal de Devolucao {DEV_VERSION}\nEntender a Tributacao {TAX_VERSION}\n',encoding='utf-8',newline='\n')
    info={
      'product':'CSM Visualizador XML','version':APP_VERSION,'build':BUILD_NAME,
      'native_key_lookup':True,'native_key_lookup_hotkey':'Ctrl+L','native_key_lookup_reuses_locator':True,'native_key_lookup_history':True,
      'native_key_lookup_action_fallback':'3.11.1','native_key_lookup_visible_header':'3.11.3','native_key_lookup_header_observer':True,
      'native_key_lookup_xml_only':True,'native_key_lookup_blocks_spreadsheets':True,'native_key_lookup_validates_access_key_in_xml':True,
      'native_key_lookup_opens_via_broker':True,'postinstall_fresh_launcher':True,
    ...
    for tok in (MARKER,'CSM_NATIVE_KEY_LOOKUP_V1','CSM_NATIVE_LOOKUP_ACTION_3111','CSM_NATIVE_LOOKUP_VISIBLE_3113','maintainLookupButton','Consultar NF-e pela chave','CSM_DIFAL_ST_MODULE_V1','CSM_DIFAL_ST_DISPLAY_3102','renderFiscalPanel','CSM_TAX_ENGINE_V1','CSM_TAX_EXPLAINER_UI_V1','CSM_TAX_EXPLAINER_ENTRYPOINT_V2','CSM_DEVOLUTION_ENTRYPOINT_V4','CSM_FOLDER_IMPORT_PROGRESS_V3'):
