...
APP_VERSION='3.9.2';MARKER='CSM_RELEASE_IDENTITY_3_9_2';DEV_VERSION='1.3.0';TAX_VERSION='1.0.2';BUILD_NAME='Entender a Tributacao 1.0.2 + Motor Fiscal de Devolucao 1.3'
OLD=r'(?:3\.7\.8|3\.8\.0|3\.8\.1|3\.9\.0|3\.9\.1)(?:\.\d+)?'
...
def main():
    ...
    if t.count(expected)<2:raise SystemExit('Modal Sobre nao esta fixado em 3.9.2')
    ...
    info={'product':'CSM Visualizador XML','version':APP_VERSION,'build':BUILD_NAME,'motor_devolucao':DEV_VERSION,'entender_tributacao':TAX_VERSION,'complete_installer':True,'base_runtime':'3.7.8','entrypoint_deterministic':True,'tax_xml_direct':True,'tax_entry_activate_document':True,'tax_no_mutation_render_loop':True}
    (csm/'build-info.json').write_text(json.dumps(info,ensure_ascii=False,indent=2),encoding='utf-8')
    final=app.read_text(encoding='utf-8')
    for tok in (MARKER,'CSM_TAX_ENGINE_V1','CSM_TAX_EXPLAINER_UI_V1','CSM_TAX_EXPLAINER_ENTRYPOINT_V2','CSM_DEVOLUTION_ENTRYPOINT_V4'):
