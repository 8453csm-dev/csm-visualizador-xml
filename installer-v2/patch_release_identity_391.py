...
APP_VERSION='3.9.1';MARKER='CSM_RELEASE_IDENTITY_3_9_1';DEV_VERSION='1.3.0';TAX_VERSION='1.0.1';BUILD_NAME='Entender a Tributação 1.0.1 + Motor Fiscal de Devolução 1.3'
OLD=r'(?:3\.7\.8|3\.8\.0|3\.8\.1|3\.9\.0)(?:\.\d+)?'
...
def main():
    ...
    if t.count(expected)<2:raise SystemExit('Modal Sobre não está fixado em 3.9.1')
    if MARKER not in t:app.write_text(t.rstrip()+f'\n// {MARKER} — {BUILD_NAME}\n',encoding='utf-8',newline='\n')
    csm.mkdir(parents=True,exist_ok=True);root=web.parent.parent
    (root/'VERSION.txt').write_text(f'CSM Visualizador XML {APP_VERSION}\nMotor Fiscal de Devolução {DEV_VERSION}\nEntender a Tributação {TAX_VERSION}\nInstalador completo\n',encoding='utf-8',newline='\n')
    info={'product':'CSM Visualizador XML','version':APP_VERSION,'build':BUILD_NAME,'motor_devolucao':DEV_VERSION,'entender_tributacao':TAX_VERSION,'complete_installer':True,'base_runtime':'3.7.8','entrypoint_deterministic':True,'tax_xml_direct':True,'tax_no_freeze_fix':True}
    (csm/'build-info.json').write_text(json.dumps(info,ensure_ascii=False,indent=2),encoding='utf-8')
    final=app.read_text(encoding='utf-8')
    for tok in (MARKER,'CSM_TAX_ENGINE_V1','CSM_TAX_EXPLAINER_UI_V1','CSM_TAX_EXPLAINER_NO_FREEZE_391','CSM_DEVOLUTION_ENTRYPOINT_V4'):
      if tok not in final:raise SystemExit('Componente ausente após identidade: '+tok)
    if re.search(r'(?i)(Versão|Versao)\s+(?:3\.7\.8|3\.8\.0|3\.8\.1|3\.9\.0)',final):raise SystemExit('Versão antiga ainda visível no app.js')
    print(f'Identidade {APP_VERSION} validada; alterações frontend: {total}')
    return 0
